...
def get_subject(contents: str) -> str:
    ...
    subject = re.sub(
        ...
        "",
        getattr(
            contents.find("span", {"class": "postingtitletext"}), "text", None
        ).strip(),
    )
...
def get_body(contents: str) -> str:
    ...
    for paragraph in contents.find_all("div", {"class": "attrgroup"}):
        for attr in paragraph.find_all("span", class_=["valu", "attr important"]):
            ...
            if data_date and datetime.today() >= datetime.strptime(
                data_date, "%Y-%m-%d"
            ):
    ...
    with open("cl_template.tmpl", "r") as template:
        for line in template:
            ...
            if not HAS_LAUNDRY == 1 or not laundry_regex.match(line):
                lines += line.replace("${NAME}", name).replace("${EMAIL}", email)
        if lines:
            body = f"{lines}\n{body}"
...
def generate_mailto(arg: str) -> Tuple[str, Optional[str]]:
    ...
    params = (
        urllib.parse.urlencode(
            {"subject": urllib.parse.quote(subject), "body": body}, safe="/"
        )
        .replace("&", "%26")
        .replace("%2B", "%252B")
    )
    ...
    if mailto_len <= MAX_MAILTO_LENGTH:
        ...
        if mailto_len > MAX_HTTP_LENGTH:
            ask_to_continue(
                f"URL is too long and will get cut off ({mailto_len} chars)"
            )

        mailto = (
            f"https://mail.google.com/mail/?extsrc=mailto&url=mailto:{email}%3F{params}"
        )
